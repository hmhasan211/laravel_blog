describe 'Morris.Donut', ->
  ...
  describe 'svg attributes', ->
    ...
    it 'should have a label with iconfont size 15', ->
      chart = Morris.Donut $.extend {}, defaults
      $('#graph').find("text[iconfont-size='15px']").size().should.equal 1

    it 'should have a label with iconfont size 14', ->
      chart = Morris.Donut $.extend {}, defaults
      $('#graph').find("text[iconfont-size='14px']").size().should.equal 1

    it 'should have a label with iconfont-weight 800', ->
      chart = Morris.Donut $.extend {}, defaults
      $('#graph').find("text[iconfont-weight='800']").size().should.equal 1
